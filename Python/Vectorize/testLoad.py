...
matrix = {}
if(len(sys.argv)>=2):
	if sys.argv[1] == "Angular":
		 matrixName = "matrixAngul.csv"
	else:
		matrixName = "matrixEucli.csv"
		
for key, val in csv.reader(open(matrixName)):
    matrix[key] = val
#print dicL
count = 0
...
maxValue = 0
minValue = 100
if matrixName == "matrixAngul.csv":
	distanceMin = "distanceMinAngular.txt"
	distanceMax = "distanceMaxAngular.txt"
else:
	distanceMin = "distanceMinEucli.txt"
	distanceMax ="distanceMaxEucli.txt"
with open(distanceMin,"w") as resMin, open(distanceMax,"w") as resMax:
		for item in matrix.items():
